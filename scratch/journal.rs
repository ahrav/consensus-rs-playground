    ...
    Journal<S, WRITE_OPS, WRITE_OPS_WORDS>
{
    ...
    fn recover_headers_on_read(completion: &mut S::Read) {
        let read: *mut Read<S, WRITE_OPS, WRITE_OPS_WORDS> =
            unsafe { container_of!(completion, Read<S, WRITE_OPS, WRITE_OPS_WORDS>, completion) };
        ...
        let chunk_bytes = &buffer[..chunk.len];
        let chunk_headers =
    ...
    fn op_maximum_headers_untrusted_raw(cluster: u128, headers: &[HeaderPrepareRaw]) -> u64 {
        ...
        for (index, header_raw) in headers.iter().enumerate() {
            let slot = Slot::new(index);
            if let Some(header) = header_ok(cluster, slot, header_raw) {
                if header.operation != Operation::RESERVED {
                    op_max = op_max.max(header.op);
                }
            }
        }
        ...
        for (index, header) in self.headers.iter().enumerate() {
            ...
            if let Some(header_ok) = header_ok(cluster, slot, header_raw) {
                if header_ok.operation != Operation::RESERVED {
                    op_max = op_max.max(header_ok.op);
                }
            }
        }
    ...
    fn torn_prepares(
        ...
        for (index, case) in cases.iter().enumerate() {
            if case.decision(solo) != RecoveryDecision::Vsr {
                continue;
            }

            let slot = Slot::new(index);

            // SlotRange.contains() does not permit head == tail.
            let within_range = if range_empty {
                index == op_prepare_max_slot.index()
            } else {
                slot_range.contains(slot)
            };
    ...
    fn recover_slots(&mut self) {
        ...
        // Determine op_max using the untrusted on-disk headers (redundant + prepares).
        let op_max_headers =
        ...
        for index in 0..SLOT_COUNT {
            ...
            if let Some(prepare_ok) = prepare {
                if prepare_ok.operation != Operation::RESERVED {
                    assert!(!self.prepare_inhabited[index]);
                    self.prepare_inhabited[index] = true;
                    self.prepare_checksums[index] = prepare_ok.checksum;
                } else {
                    assert!(!self.prepare_inhabited[index]);
                    assert!(self.prepare_checksums[index] == 0);
                }
            }
        }
    ...
    fn remove_entry(&mut self, slot: Slot, cluster: u128) {
    ...
    fn recover_slot(
        ...
        match decision {
            ...
            RecoveryDecision::CutTorn => {
                ...
                let header = header.expect("CUT_TORN requires a valid redundant header");
                assert!(header.operation != Operation::RESERVED);
            ...
            RecoveryDecision::Unreachable => {
                panic!("recovery case should be unreachable: {}", case.label);
            }
        }
    ...
    fn recover_done(&mut self) {
        let callback = match self.status {
            Status::Recovering { callback } => callback,
